...
Train XGBoost on synthetic data (Toronto + Ottawa) only.
Evaluate on a held-out slice of REAL Lanark data that the model never sees during training.

Usage:
    python condition_b_xgboost.py --toronto toronto.csv --ottawa ottawa.csv --lanark lanark.csv
"""
...
def main(toronto_path, ottawa_path, lanark_path, val_size=0.2):
    ...
    # (Lanark's "train" split isn't used in Condition B itself, but keeping the
    # split identical across all 4 conditions is what makes results comparable.)
    ...
    print(f"Training pool (synthetic only): {len(X_train)} rows, "
          f"{y_train.sum()} positives ({y_train.mean():.1%})")
    print(f"Lanark validation (real, held out): {len(X_val)} rows, "
    ...
    evaluate(model, X_train, y_train, "Tier 1 - synthetic training pool (sanity check only)")

    # --- Evaluate: Tier 2 - the real result ---
    results = evaluate(model, X_val, y_val, "Tier 2 - REAL Lanark holdout (Condition B, zero-shot)")

    # --- Bootstrap CI on the real Lanark holdout ---
    ci = bootstrap_ci(model, X_val, y_val, n_boot=1000)
